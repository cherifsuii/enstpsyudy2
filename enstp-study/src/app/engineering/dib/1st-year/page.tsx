...
const SubjectCard = ({ subject }: { subject: Subject }) => {
  const isComingSoon = subject.resources.some(r => r.label.toLowerCase().includes('coming soon'));

  if (isComingSoon) {
    return (
      <div className="bg-gray-100 p-6 rounded-lg shadow-md">
        <h3 className="text-2xl font-bold text-gray-500 mb-4">{subject.name}</h3>
...
export default function DIBFirstYearPage() {
  const allData = getParsedData();
  const semester1 = allData['1ere_annee_dib_semestre_1'] || [];
  const semester2 = allData['1ere_annee_dib_semestre_2'] || [];

  return (
    <div>
      <h1 className="text-4xl font-bold mb-8 text-center">DIB - 1st Year</h1>
